...
# DATA_DIR = '/home/yliu31/clayTrials/TEST_DAY_1/data/2020-11-13-00'
# DATA_DIR = '/desktop/TCP/test'
# DATA_DIR = './11-16-00/'
...
def parse_directory(directory):
    ...
    expect directory like "11-16-00/mlcnetD.cs.wpi.edu_pcc_Proxy2_1G_5"
    """
...
def summary(df, directory=None, start_bytes=0, end_bytes=1e9 * 10):
    ...
    if (directory):
        host, protocol, proxy = parse_directory(directory)
    else:
        host, protocol, proxy = '', '', ''

    if df.empty:
        # throughput_quantiles, rtt_quantiles, host, protocol, proxy, start_time, loss
        return {}, {}, host, protocol, proxy, 0, 0
    ...
    print(rtt_quantiles)
    return throughput_quantiles, rtt_quantiles, host, protocol, proxy, start_time, loss
...
def main(DATA_DIR=DATA_DIR):
    ...
    for local, remote, dir in all_pcaps():
        ...
        throughput_quantiles, _rtt_quantiles, host, protocol, proxy, start_time, _ = results[0]
        steady_throughput_quantiles, _steady_rtt_quantiles, _, _, _, steady_start_time, _ = results[1]
        _, _rtt_quantiles, _, _, _, _, loss = results[2]
        _, _steady_rtt_quantiles, _, _, _, _, steady_loss = results[3]
        startup_throughput_quantiles, _startup_rtt_quantiles, _, _, _, startup_start_time, _ = results[4]

        rtt_quantiles = find_rtt_quantiles(pcap_to_csv(remote))
        steady_rtt_quantiles = find_rtt_quantiles(pcap_to_csv(remote), steady=True)
        startup_rtt_quantiles = find_rtt_quantiles(pcap_to_csv(remote), startup=True)
        ...
        throughput_quantiles['proxy'] = proxy
        ...
        steady_throughput_quantiles['proxy'] = proxy
        ...
        startup_throughput_quantiles['proxy'] = proxy
        ...
        steady_rtt_quantiles['proxy'] = proxy
        ...
        startup_rtt_quantiles['proxy'] = proxy
...
def timeslice(filename):
    ...
    host, protocol, proxy = parse_directory(dirname)
